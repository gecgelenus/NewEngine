...
class RenderBatch{
    ...
    public:
        RenderBatch();
        RenderBatch(const vk_ctx& p_ctx, const std::string& p_name);
        void __RenderBatch();
        ~RenderBatch();
        ...
        void processNode(tinygltf::Model& model, std::string& path, const tinygltf::Node& node, ObjectTransformation parentTransform);
